...
export const VehicleHistoryTabs: React.FC<VehicleHistoryTabsProps> = ({
  ...
  const [tabsLabelStyling, setTabsLabelStyling] = useState({
    key: "Trip",
    style: "border-dark-glow-green border-opacity-100",
  });
  ...
  const { customRangeSelected } = useSelector(
  ...
  const hasItineraryData =
    ...
    (data.data.length > 1 ||
      (data.data.length === 1 && data.data[0]?.totalTime !== ""));
  ...
  useEffect(
    () => {
      ...
      if (
        ...
        if (
          !newItems.find((item) => item.key === "Trip") &&
          !isReportsExpanded
        ) {
          // remove all
      ...
      if (isReportsExpanded) {
        newItems = newItems.filter(
          (item) => item.key !== "Trip" && item.key !== "Alerts",
        );
      }
