...
export const VehicleOverviewCard = ({
  ...
  const targetStatusInfo = useMemo(() => {
    const gpsInfo = vehicleData.GPSInfo as any;
    const elockInfo = vehicleData.ELOCKInfo as any;
    ...
    const isAvailable = (info: any): boolean => {
      return Boolean(
        info &&
        (info.vehId || info.vId || info.vehReg || info.mode || info.gpstime),
      );
    };

    const isWorking = (info: any): boolean => {
      return isAvailable(info) && info.gps_fix !== 0;
    };

    const gpsAvailable = isAvailable(gpsInfo);
    const elockAvailable = isAvailable(elockInfo);

    const gpsHasA = hasSpaceA(gpsInfo);
    const elockHasA = hasSpaceA(elockInfo);

    const extractInfo = (info: any) => ({
      mode: (info?.mode || vehicleData.gpsDtl.mode || "").toString(),
      modeTime: (
        info?.modetimenew ||
        info?.modeTime ||
        info?.modetime ||
      ...
      speed:
        typeof info?.speed === "number" ? info.speed : vehicleData.gpsDtl.speed,
    });

    if (!gpsAvailable && !elockAvailable) {
      return {
        mode: vehicleData.gpsDtl.mode,
        modeTime: vehicleData.gpsDtl.modeTime,
        speed: vehicleData.gpsDtl.speed,
      };
    }

    let nonADevice: any = null;
    let aDevice: any = null;

    if (gpsAvailable && !gpsHasA) nonADevice = gpsInfo;
    else if (gpsAvailable && gpsHasA) aDevice = gpsInfo;
    ...
    // 2. If that is not working, then show A one if working
    if (aDevice && isWorking(aDevice)) {
      return extractInfo(aDevice);
    }

    // 3. If both are not working, then show not A one
    if (nonADevice) {
      return extractInfo(nonADevice);
    }
  ...
  const currentModeArr = (targetStatusInfo.mode || "STOPPED").split("");
  ...
  const { data: ambulanceData } = useGetAmbulanceEmployeesQuery(undefined, {
  ...
  const formatAddress = (value?: string) => value?.replaceAll("_", " ") || "";

  const parseTrackingTime = (value?: string) => {
    if (!value) return 0;

    const parsed = moment(
      value,
      ["D MMMM YYYY HH:mm", "DD MMMM YYYY HH:mm", moment.ISO_8601],
      true,
    );

    return parsed.isValid() ? parsed.valueOf() : new Date(value).getTime() || 0;
  };

  const getLatestTrackingSource = () => {
    const gpsTime = parseTrackingTime(vehicleData.GPSInfo?.gpstime);
    const elockTime = parseTrackingTime(vehicleData.ELOCKInfo?.gpstime);

    if (gpsTime >= elockTime) {
      return vehicleData.GPSInfo || vehicleData.ELOCKInfo || null;
    }

    return vehicleData.ELOCKInfo || vehicleData.GPSInfo || null;
  };

  const gpsInfoHasA = vehicleData.GPSInfo?.vehReg?.endsWith(" A");
  const elockInfoHasA = vehicleData.ELOCKInfo?.vehReg?.endsWith(" A");
  const showCabinWarningInElock = gpsInfoHasA && !elockInfoHasA;

  const getGpsLocation = () => {
    if (gpsInfoHasA && vehicleData.ELOCKInfo?.addr) {
      return formatAddress(vehicleData.ELOCKInfo.addr);
    }

    if (!gpsInfoHasA && vehicleData.GPSInfo?.addr) {
      return formatAddress(vehicleData.GPSInfo.addr);
    }

    if (elockInfoHasA && vehicleData.GPSInfo?.addr) {
      return formatAddress(vehicleData.GPSInfo.addr);
    }

    if (vehicleData.ELOCKInfo?.addr) {
      return formatAddress(vehicleData.ELOCKInfo.addr);
    }

    return formatAddress(vehicleData.gpsDtl.latLngDtl.addr);
  };

  const getElockLocation = () => {
    if (gpsInfoHasA && vehicleData.GPSInfo?.addr) {
      return formatAddress(vehicleData.GPSInfo.addr);
    }

    if (elockInfoHasA && vehicleData.ELOCKInfo?.addr) {
      return formatAddress(vehicleData.ELOCKInfo.addr);
    }
  ...
  const getGpsCoordinates = () => {
    const latestSource = getLatestTrackingSource();

    if (latestSource?.lat && latestSource?.lng) {
      return {
        lat: latestSource.lat,
        lng: latestSource.lng,
      };
    }
  ...
  const getElockCoordinates = () => {
    if (vehicleData.ELOCKInfo?.lat && vehicleData.ELOCKInfo?.lng) {
      return {
    ...
    if (
      vehicleData.GPSInfo?.vehReg?.endsWith(" A") &&
      vehicleData.GPSInfo?.lat &&
      vehicleData.GPSInfo?.lng
    ) {
      return {
  ...
  const getLatestReceivedTime = () => {
    const gpsTime = parseTrackingTime(vehicleData.GPSInfo?.gpstime);
    const elockTime = parseTrackingTime(vehicleData.ELOCKInfo?.gpstime);

    if (gpsTime || elockTime) {
      return gpsTime >= elockTime
        ? vehicleData.GPSInfo?.gpstime || vehicleData.ELOCKInfo?.gpstime || ""
        : vehicleData.ELOCKInfo?.gpstime || vehicleData.GPSInfo?.gpstime || "";
    }
  ...
  const stablePOI = useMemo(() => {
    ...
    if (currentPOI === "Inside Geofence") {
      const primaryAddress = getGpsLocation();
          ...
          <div className="flex flex-col items-baseline gap-0.5 overflow-hidden relative">
            ...
            <div className="flex items-center gap-1 mb-1">
              <div className="font-bold text-gray-600 w-6 relative overflow-visible">
                <Tooltip
                  title={
                    getGpsLocation() ? "GPS location" : "GPS location not fixed"
                  }
                  placement="left"
                  mouseEnterDelay={1}
                >
                  ...
                  >
                    <div className="border border-gray-300 px-2 py-3 rounded-lg text-xs w-20 h-[84px] min-w-[80px]">
                      <div className="font-bold">
                        {`${getGpsCoordinates().lat.toFixed(
                          4,
                        )} ${getGpsCoordinates().lng.toFixed(4)}`}
                      </div>
