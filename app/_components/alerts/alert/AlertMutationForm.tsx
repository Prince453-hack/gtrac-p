import { useLazyCreateServiceOrDocumentAlertQuery, useLazyEditServiceOrDocumentAlertQuery } from '@/app/_globalRedux/services/yatayaat';
...
export const AlertMutationForm = ({
	handleModalCancel,
	modalViewToggle,
	selectedAlert,
	...
	const [getAlertManagementTrigger] = useLazyGetAlertsManagementQuery();
	...
	useEffect(() => {
		...
		if (modalViewToggle === 'EDIT') {
			setSelectedAlertType(selectedAlert?.SetAlertType || 'OverSpeed');

			form.setFieldsValue({
				SetAlertValue: selectedAlert?.SetAlertType,
				selectVehicle: selectedAlert?.VehicleNo.split(','),
				SetSpeedValue: selectedAlert?.SetSpeedValue,
				SetTempFromValue: selectedAlert?.SetTempFrom,
				SetTempToValue: selectedAlert?.SetTempTo,
				Email: selectedAlert?.email_id,
				MobileNo: selectedAlert?.mobile_No,
			});
			if (selectedAlert?.VehicleNo !== 'All') {
				let allSelectedVehicles = '';
				selectedAlert?.VehicleNo.split(',').forEach((vehicleReg) => {
					let tempVehicle = vehicleOptions.find((vehicleOption) => vehicleOption.label === vehicleReg);
					allSelectedVehicles += `${tempVehicle?.value}##${tempVehicle?.label}, `;
				});

				setSelectedVehicle(allSelectedVehicles);
