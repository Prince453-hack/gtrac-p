...
export const View = () => {
  ...
  const getVehicleStatusDisplay = (
    ...
    return getVehicleStatusFromOriginal(row.original);
  };
  ...
  const getVehiclesNotWorking = (row: Row<VehicleDataWithAnkurTravelsData>) => {
    return getVehicleStatusDisplay(row) === "NOT WORKING";
  };
  ...
  const columns: ColumnDef<VehicleDataWithAnkurTravelsData>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <p className={`${getVehiclesNotWorking(row) ? "text-red-700 " : ""}`}>
            {gpsTimeToDisplay ? moment(gpsTimeToDisplay).format("Do MMM, YYYY, HH:mm") : ""}
          </p>
        );
      },
      header: "Last Update",
    ...
    ...ankurCarrier(),
    {
      ...
      accessorFn: (row: VehicleDataWithAnkurTravelsData) =>
        getVehicleStatusFromOriginal(row),
      cell: ({ row }) => (
        <p className={`${getVehiclesNotWorking(row) ? "text-red-700 " : ""}`}>
          {getVehicleStatusDisplay(row)}
        </p>
      ),
      header: "Status",
      footer: (props) => props.column.id,
              ...
              {(() => {
                ...
                return (
                  <iframe
                    title="map"
                    loading="lazy"
                    src={`https://gtrac.in/newtracking/mapnewwindow.php?lat=${mapLat}&long=${mapLong}&vehicle=${selectedVehicle?.vehReg}&token=${groupId}&userid=${Number(userId)}`}
