...
const remoteUploadUrl = process.env.ACCIDENTAL_IMAGE_UPLOAD_URL || "";
const remoteDeleteUrl = process.env.ACCIDENTAL_IMAGE_DELETE_URL || "";
const remoteFetchUrl = process.env.ACCIDENTAL_IMAGE_FETCH_URL || "";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // If a remote upload endpoint is configured, forward the file there.
    if (remoteUploadUrl) {
      try {
        const forwardForm = new FormData();
        forwardForm.append("file", file, file.name);
        forwardForm.append("vehicleId", vehicleId);
        forwardForm.append("vehicleNumber", vehicleNumber);
        forwardForm.append("timestamp", timestamp);

        const res = await fetch(remoteUploadUrl, {
          method: "POST",
          body: forwardForm,
        });
        ...
        if (!res.ok) {
          ...
          throw new Error(`Remote upload failed: ${res.status} ${txt}`);
        }

        const data = await res.json().catch(() => ({}));
        // Prefer `url` or `filePath` from remote response
        const remotePath = data?.url || data?.filePath || data?.path || null;
        ...
        return NextResponse.json(
          { error: "Failed to upload image to remote server", details: String(err) },
          { status: 502 }
        );
      }
    }
